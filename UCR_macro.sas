...
** Set libref destination for output files: 
librname ucr ;



** START MACRO **;
** macro variable &yr is YY format (by convention through this code, yr is YY and year is YYYY)

%macro UCR_read_and_aggregate_by_sate (yr=, ucr_filepath=, census_filepath=);

****************************************
///////////////////\\\\\\\\\\\\\\\\\\\\\
	STEP 1: 
	Read and clean Census population 
	estimates
\\\\\\\\\\\\\\\\\\\/////////////////////
****************************************;
* Import census population estimates;
PROC IMPORT OUT= census_raw  
            DATAFILE= &census_filepath
            DBMS=EXCEL5 REPLACE;
			GETNAMES=YES;
RUN;
...
data census_clean;
	...
	*drop any cases for wrong year
	if year = 1900 + &yr;


	* correcting counts by age;
	* note variable naming convention; 
	* variables a0515 (for example) is population count of all people ages 5 to 17;
	*counts are difided by 1000 in raw data
	array a1 all a0004	a0517	a1824	a2544	a4564	a65up;
	...
	* setting correct YYYY year;
	if b then year = 1900 + year;

	* setting correct numeric state indicator (FIPS State number);
	* for merging with UCR;
	if state= 'AK' then state_n=50; 
	if state= 'AL' then state_n=01; 
	if state= 'AR' then state_n=03; 
	if state= 'AS' then state_n=54; 
	if state= 'AZ' then state_n=02; 
	if state= 'CA' then state_n=04; 
	if state= 'CO' then state_n=05; 
	if state= 'CT' then state_n=06; 
	if state= 'CZ' then state_n=52; 
	if state= 'DC' then state_n=08; 
	if state= 'DE' then state_n=07; 
	if state= 'FL' then state_n=09; 
	if state= 'GA' then state_n=10; 
	if state= 'GM' then state_n=55; 
	if state= 'HI' then state_n=51; 
	if state= 'IA' then state_n=14; 
	if state= 'ID' then state_n=11; 
	if state= 'IL' then state_n=12; 
	if state= 'IN' then state_n=13; 
	if state= 'KS' then state_n=15; 
	if state= 'KY' then state_n=16; 
	if state= 'LA' then state_n=17; 
	if state= 'MA' then state_n=20; 
	if state= 'MD' then state_n=19; 
	if state= 'ME' then state_n=18; 
	if state= 'MI' then state_n=21; 
	if state= 'MN' then state_n=22; 
	if state= 'MO' then state_n=24; 
	if state= 'MS' then state_n=23; 
	if state= 'MT' then state_n=25; 
	if state= 'NE' then state_n=26; 
	if state= 'NC' then state_n=32; 
	if state= 'ND' then state_n=33; 
	if state= 'NH' then state_n=28; 
	if state= 'NJ' then state_n=29; 
	if state= 'NM' then state_n=30; 
	if state= 'NV' then state_n=27; 
	if state= 'NY' then state_n=31; 
	if state= 'OH' then state_n=34; 
	if state= 'OK' then state_n=35; 
	if state= 'OR' then state_n=36; 
	if state= 'PA' then state_n=37; 
	if state= 'PR' then state_n=53; 
	if state= 'RI' then state_n=38; 
	if state= 'SC' then state_n=39; 
	if state= 'SD' then state_n=40; 
	if state= 'TN' then state_n=41; 
	if state= 'TX' then state_n=42; 
	if state= 'UT' then state_n=43; 
	if state= 'VI' then state_n=62; 
	if state= 'VA' then state_n=45; 
	if state= 'VT' then state_n=44; 
	if state= 'WA' then state_n=46; 
	if state= 'WI' then state_n=48; 
	if state= 'WV' then state_n=47; 
	if state= 'WY' then state_n=49; 

	*Combining population counts by age group;
	a0017 = sum (of a0004 a0517); * children;
	a0524 = sum (of a0517 a1824);	* school aged children;
...
proc sort data=census_clean; by year state_n; run;
...
data ucr.header&yr;
	...
	*if header, we proceed;
	...
	if off = 000 then output ucr.header&yr;
	end;
run;
...
data ucr.detail&yr;
	...
	if off NE 000 then output ucr.detail&yr;
	run;
...
proc sort data=ucr.detail&yr ; by oricode; 
proc sort data=ucr.header&yr ; by oricode; 
run;

data d1;
	merge 	ucr.detail&yr
			ucr.header&yr (keep=oricode curpop);
	by oricode;
	run;
...
****************************************
///////////////////\\\\\\\\\\\\\\\\\\\\\
	STEP 7: Aggregate data by state
\\\\\\\\\\\\\\\\\\\/////////////////////
****************************************;
...
data d4;
	...
	*Set variable order for use in MPlus;
	retain 
	...
	* note that array rows are offense categories;
...
///////////////////\\\\\\\\\\\\\\\\\\\\\
	STEP 8: 
	Merge state-level UCR info + Census 
	Calculate offense rates;
\\\\\\\\\\\\\\\\\\\/////////////////////
****************************************;
data ucr.aggt&yr._&fn;
	merge   d4
			census_clean;
	by state_n;
	...
	*no need for 2D array, so just running straight through;
	do i = 1 to 145;
		...
		*Calculating juvenile rates dividing by number of children in state;
		if i < 41 then a4{i} = a3{i} / (a0017/1000);
